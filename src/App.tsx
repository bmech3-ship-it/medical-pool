...
const addDays = (d: string, n: number): string => { const x = new Date(d); x.setDate(x.getDate() + n); return x.toISOString().slice(0, 10); };
const isValidDate = (d: any) => d instanceof Date && !isNaN(d.getTime());
const formatDate = (d?: string | null): string => { if (!d) return ""; const x = new Date(d); return isValidDate(x) ? x.toLocaleDateString() : ""; };
const daysBetween = (a: string, b: string | Date = new Date()): number => { const d1 = new Date(a), d2 = new Date(b); if (!isValidDate(d1) || !isValidDate(d2)) return 0; return Math.floor((d2.getTime() - d1.getTime()) / 86400000); };
...
const Text = ({ label, value, onChange, type = "text", placeholder, required }: { label: string; value: any; onChange?: (v: string) => void; type?: string; placeholder?: string; required?: boolean; }) => (
  <label className="block">
    <span className="block text-sm font-medium mb-1 text-slate-700">{label}{required ? " *" : ""}</span>
    <input type={type} value={value} onChange={(e) => onChange && onChange(e.target.value)} placeholder={placeholder}
      required={required} className="w-full px-3 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-200 bg-white" />
  </label>
);
...
function SignaturePad({ value, onChange, height = 120 }: { value?: string | null; onChange?: (v: string | null) => void; height?: number; }) {
  ...
  useEffect(() => {
    const c = ref.current; if (!c) return; const ctx = c.getContext('2d'); if (!ctx) return; ctx.lineWidth = 2; ctx.lineCap = 'round';
    const pos = (e: any) => { const r = c.getBoundingClientRect(); const t = e.touches?.[0]; const x = (t ? t.clientX : e.clientX) - r.left; const y = (t ? t.clientY : e.clientY) - r.top; return { x, y }; };
  ...
  const clear = () => { const c = ref.current; if (!c) return; const ctx = c.getContext('2d'); if (!ctx) return; ctx.clearRect(0, 0, c.width, c.height); onChange && onChange(null); };
  return (
    <div>
...
export default function App() {
  ...
  // master data
  const [brands, setBrands] = useState<string[]>(['Philips', 'GE']);
  const [models, setModels] = useState<{ brand: string; name: string; }[]>([{ brand: 'Philips', name: 'IntelliVue' }, { brand: 'GE', name: 'Carescape' }]);
  const [vendors, setVendors] = useState<string[]>(['MedSupply Co.']);
  const [depts, setDepts] = useState<string[]>(['ER', 'ICU', 'OR', 'WARD']);
  ...
  const [orgName, setOrgName] = useState<string>(() => localStorage.getItem('org_name') || 'Hospital Name');
  const [reportLogo, setReportLogo] = useState<string>(() => localStorage.getItem('report_logo') || '');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
          ...
          <nav className="ml-auto flex flex-wrap gap-2">
            {(Object.keys(TABS) as Array<typeof tab>).map((id) => (
              <button key={id} onClick={() => setTab(id)}
                className={"px-3 py-2 rounded-xl border inline-flex items-center gap-2 transition " + (tab === id ? 'bg-blue-600 text-white border-blue-700 shadow-sm' : 'bg-white hover:bg-slate-50')}> {TABS[id].icon} {TABS[id].label}
              </button>
            ))}
          </nav>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-6 text-slate-800">
        ...
        {tab === 'assets' && (
          <Assets brands={brands} setBrands={setBrands} models={models} setModels={setModels} vendors={vendors} setVendors={setVendors} assets={assets} onCreateAsset={createAsset} onDeleteAsset={deleteAsset} onUpdateAsset={updateAsset} />
        )}
        {tab === 'borrow' && (
...
function Assets({ brands, setBrands, models, setModels, vendors, setVendors, assets, onCreateAsset, onDeleteAsset, onUpdateAsset }: any) {
  ...
  const confirmAddBrand = () => { const v = newBrand.trim(); if (!v) return; if (!brands.includes(v)) setBrands([...brands, v]); if (editingId) setEditForm((p: any) => ({ ...p, brand: v, model: '' })); else setForm({ ...form, brand: v, model: '' }); setNewBrand(''); setAddingBrand(false); };
  const confirmAddModel = () => { const currentBrand = editingId ? editForm.brand : form.brand; if (!currentBrand) return alert('เลือกยี่ห้อก่อน'); const v = newModel.trim(); if (!v) return; if (!models.find((m: any) => m.name === v && m.brand === currentBrand)) setModels([...models, { brand: currentBrand, name: v }]); if (editingId) setEditForm((p: any) => ({ ...p, model: v })); else setForm({ ...form, model: v }); setNewModel(''); setAddingModel(false); };
  const confirmAddVendor = () => { const v = newVendor.trim(); if (!v) return; if (!vendors.includes(v)) setVendors([...vendors, v]); if (editingId) setEditForm((p: any) => ({ ...p, vendor: v })); else setForm({ ...form, vendor: v }); setNewVendor(''); setAddingVendor(false); };

  const submit = async (e: React.FormEvent) => {
    e.preventDefault(); setErr(''); const f = form as any;
  ...
  const BrandField = ({ isEdit }: { isEdit?: boolean }) => (
    <label className="block">
      ...
      <div className="flex gap-2 items-start">
        <select value={isEdit ? editForm.brand : form.brand} onChange={(e) => isEdit ? setEditForm((p: any) => ({ ...p, brand: e.target.value, model: '' })) : setForm({ ...form, brand: e.target.value, model: '' })} className="w-full px-3 py-2 border rounded-xl">
  ...
  const ModelField = ({ isEdit }: { isEdit?: boolean }) => (
    <label className="block">
      ...
      <div className="flex gap-2 items-start">
        <select value={isEdit ? editForm.model : form.model} onChange={(e) => isEdit ? setEditForm((p: any) => ({ ...p, model: e.target.value })) : setForm({ ...form, model: e.target.value })} className="w-full px-3 py-2 border rounded-xl">
          <option value="">-- เลือก --</option>
  ...
  const VendorField = ({ isEdit }: { isEdit?: boolean }) => (
    <label className="block">
      ...
      <div className="flex gap-2 items-start">
        <select value={isEdit ? editForm.vendor : form.vendor} onChange={(e) => isEdit ? setEditForm((p: any) => ({ ...p, vendor: e.target.value })) : setForm({ ...form, vendor: e.target.value })} className="w-full px-3 py-2 border rounded-xl">
          <option value="">-- เลือก --</option>
    ...
    <Card>
      ...
      <div className="overflow-auto max-h-96 border rounded-xl">
        <table className="min-w-full text-sm">
          <thead className="bg-slate-50 sticky top-0"><tr>{["Asset ID", "ID CODE", "ชื่อ", "ยี่ห้อ/รุ่น", "Serial", "วันที่ซื้อ", "ราคา", "บริษัท", "#"].map((h) => <th key={h} className="text-left px-3 py-2 border-b">{h}</th>)}</tr></thead>
          <tbody>
            {assets.map((a: any) => (
              <tr key={a.asset_id} className="odd:bg-white even:bg-slate-50 align-top hover:bg-slate-100/60">
                {editingId === a.asset_id ? (
                  <>
                    ...
                    <td className="px-3 py-2 border-b"><VendorField isEdit /></td>
                    <td className="px-3 py-2 border-b text-right space-x-2">
                      <Button variant="success" size="sm" onClick={saveEdit}><CheckCircle2 size={16}/>บันทึก</Button>
...
function Borrow({ assets, depts, setDepts, onCreateBorrow, activeIds = [] }: any) {
  ...
  // Inline add for Department (replace prompt)
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const rec = { id: uid(), asset_id: asset.asset_id, asset_name: asset.name, ...form, end_date: form.end_date || null, returned_at: null, borrower_sign: sign, created_at: new Date().toISOString() };
...
function Return({ borrows, onReturn, onUpdateBorrow }: any) {
  ...
  const list = useMemo(() => !kw ? active : active.filter((r: any) => { const k = kw.toLowerCase(); return r.asset_id.toLowerCase().includes(k) || (r.asset_name || '').toLowerCase().includes(k) || (r.borrower_name || '').toLowerCase().includes(k); }), [kw, active]);
...
function Report({ borrows, depts, orgName, reportLogo }: { borrows: any[]; depts: string[]; orgName: string; reportLogo: string; }) {
  ...
  const filtered = useMemo(() => borrows.filter((r: any) => { const sd = new Date(r.start_date); if (from && sd < new Date(from)) return false; if (to && sd > new Date(to)) return false; if (dept && (r.borrower_dept || '') !== dept) return false; return true; }), [borrows, from, to, dept]);
  ...
  const exportXLSX = async () => {
    if (xlLoading) return;
    // ถ้าไม่มีข้อมูลหลังกรอง ให้แจ้งเตือนและไม่ต้องสร้างไฟล์ว่าง
    ...
    try {
      ...
      const aoa: any[][] = [
        ...
        [''],
        // ให้หัวตารางครบ 10 คอลัมน์ เหมือน PDF/ไฟล์สำรอง
      ...
      filtered.forEach((r:any)=>{
        aoa.push([
          ...
          '' // ลายเซ็น: ไม่รองรับใน .xlsx (ต้องปลั๊กอินเชิงพาณิชย์ของ SheetJS)
        ]);
      });
  ...
  const printPDF = () => {
    const rows = filtered.map((r: any) => (`<tr><td>${r.asset_id}</td><td>${r.asset_name || ''}</td><td>${r.borrower_name || ''}</td><td>${r.borrower_dept || ''}</td><td>${r.lender_name || ''}</td><td>${formatDate(r.start_date)}</td><td>${r.end_date ? formatDate(r.end_date) : ''}</td><td>${r.returned_at ? formatDate(r.returned_at) : ''}</td><td>${r.returned_at ? daysBetween(r.start_date, r.returned_at) : daysBetween(r.start_date)}</td><td>${r.borrower_sign ? `<img src='${r.borrower_sign}' style='height:28px'/>` : '-'}</td></tr>`)).join('');
    ...
    const w = window.open('', '_blank'); if (!w) { alert('เบราว์เซอร์บล็อกป๊อปอัป กรุณาอนุญาต แล้วลองใหม่'); return; }
  ...
  return (
    <Card>
      ...
      <div className="overflow-auto max-h-[28rem] border rounded-xl">
        <table className="min-w-full text-sm"><thead className="bg-slate-50 sticky top-0"><tr>{["Asset ID", "ชื่อเครื่อง", "ผู้ยืม", "แผนก", "ผู้ให้ยืม", "เริ่มยืม", "กำหนดคืน", "คืนจริง", "ระยะเวลา(วัน)", "ลายเซ็น"].map(h => (<th key={h} className="text-left px-3 py-2 border-b">{h}</th>))}</tr></thead>
          <tbody>
            {filtered.map((r: any) => (
...
function ActiveLoans({ borrows, compact }: { borrows: any[]; compact?: boolean; }) {
  ...
  return (
    <div className="overflow-auto max-h-96 border rounded-xl">
      <table className="min-w-full text-sm">
        <thead className="bg-slate-50 sticky top-0"><tr>{["Asset ID", "ชื่อเครื่อง", "ผู้ยืม", "แผนก", "เริ่มยืม", "กำหนดคืน", "วันสะสม", compact ? "" : "ลายเซ็น"].filter(Boolean).map(h => (<th key={String(h)} className="text-left px-3 py-2 border-b">{h as any}</th>))}</tr></thead>
        <tbody>
          {rows.map((r: any) => { const od = daysBetween(r.start_date) >= 14; return (
...
function Settings({ orgName, setOrgName, reportLogo, setReportLogo }: { orgName: string; setOrgName: (v: string)=>void; reportLogo: string; setReportLogo: (v: string)=>void; }) {
  ...
  const testSupabase = async () => { setSbStatus('กำลังทดสอบ...'); try { const base = supabaseUrl.replace(/\/$/, ''); const res = await fetch(base + '/auth/v1/settings', { headers: { apikey: supabaseAnon } }); setSbStatus(res.ok ? 'เชื่อมต่อได้ ✅' : `เชื่อมต่อไม่ได้ (${res.status})`); } catch (e: any) { setSbStatus('เชื่อมต่อไม่ได้ ❌: ' + (e?.message || 'Unknown error')); } };

  useEffect(()=>{ localStorage.setItem('org_name', orgName); },[orgName]);
  useEffect(()=>{ localStorage.setItem('report_logo', reportLogo); },[reportLogo]);
